...
> **归零（ZerQ）质量管理指导 Skill · 对外发布版本 1.0.1**
...
归零（ZerQ）是面向**全行业质量管理与问题归零**的指导 Skill：从现场危机处置、问题归零（双归零）到体系治理评估，从知识学习到行业拓展，覆盖多类意图 × 多类领域，按场景路由动态组合输出。中文名「归零」源自质量管理的"问题归零 / 双归零"内核。
...
### 5.2 路径归一化（零硬编码）
- `scripts/paths.py`：ZerQ 内部路径单一真源（env `QCM_ROOT` > `__file__` 推导 > 默认）
...
⑤ 嵌套链接 ⑥ 组件容量 ⑦ ZerQ 硬编码 ⑧ plugins 存在性 ⑨ 跨 Skill 硬编码
⑩ ref 存在 ⑫ rglob 递归扫描
...
ZerQ 内置 5 个定时自动化任务，分「回写型 / 只读评估 / 混合」三类。回写型任务会将运行结论回写 ZerQ 资产（词库、关联热度、归档观察等），是体系"自演化"的驱动面。
...
- 建议将 venv 解释器固化为 ZerQ 所有 automation 的默认运行环境。
...
> **核心设计原则**：4 层级架构 + 5 原则 + 15 章协议 + 4 形态 + 场景路由消费 + 组件池三机制 + 文件层治理 = ZerQ 终极架构。
